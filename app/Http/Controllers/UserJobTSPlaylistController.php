    ...
    public function update($id, UpdateUserJobTSPlaylistRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($jobTSPlaylistAudios as $jobTSPlaylistAudio)
                {
                    $userJobTSPlaylistAudioInserts = DB::table('user_job_t_s_p_audios')->where('j_t_s_p_a_id', $jobTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userJobTSPlaylist -> permissions]);
