...
class SharedProfileNoteController extends AppBaseController
{
    ...
    public function store(CreateSharedProfileNoteRequest $request)
    {
        if(Auth::user() != null)
        {
            if(isset(Auth::user()->id))
            {
                ...
                DB::table('recent_activities')->insert(['name' => $sharedProfileNote -> name, 'status' => 'active', 'type' => 's_p_n_c', 'user_id' => $user_id, 'entity_id' => $sharedProfileNote -> id, 'created_at' => $now]);
    ...
    public function show($id)
    {
        ...
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $sharedProfileNote -> user_id || $isShared)
            {
                ...
                $sharedProfileNoteViewsList = DB::table('users')->join('shared_profile_note_views', 'users.id', '=', 'shared_profile_note_views.user_id')->where('s_p_n_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                $sharedProfileNoteUpdatesList = DB::table('users')->join('shared_profile_note_updates', 'users.id', '=', 'shared_profile_note_updates.user_id')->where('s_p_n_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                ...
                return view('shared_profile_notes.show')
                    ...
                    ->with('sharedProfileNoteCommentResponses', $shared_profile_note_comment_responses)
                    ->with('sharedProfileNoteCommentResponseUsers', $shared_profile_note_comment_response_users)
                    ->with('sharedProfileNoteViewsList', $sharedProfileNoteViewsList)
                    ->with('sharedProfileNoteUpdatesList', $sharedProfileNoteUpdatesList)
                    ->with('text', $text)
                    ->with('user', $user)
                    ->with('user_id', $user_id)
                    ->with('now', $now)
    ...
    public function update($id, UpdateSharedProfileNoteRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newSharedProfileNote = $this->sharedProfileNoteRepository->update($request->all(), $id);

                DB::table('shared_profile_note_updates')->insert(['actual_name' => $newSharedProfileNote -> name, 'past_name' => $sharedProfileNote -> name, 'datetime' => $now, 's_p_n_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $sharedProfileNote -> name, 'status' => 'active', 'type' => 's_p_n_u', 'user_id' => $user_id, 'entity_id' => $sharedProfileNote -> id, 'created_at' => $now]);
    ...
    public function destroy($id)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                DB::table('recent_activities')->insert(['name' => $sharedProfileNote -> name, 'status' => 'active', 'type' => 's_p_n_d', 'user_id' => $user_id, 'entity_id' => $sharedProfileNote -> id, 'created_at' => $now]);
