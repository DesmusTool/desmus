...
class PublicAudioController extends AppBaseController
{
    ...
    public function store(CreatePublicAudioRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "audios/public_audios/audio_" . $publicAudio -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        {       
            ...
            $publicAudioViewsList = DB::table('users')->join('public_audio_views', 'users.id', '=', 'public_audio_views.user_id')->where('public_audio_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicAudioUpdatesList = DB::table('users')->join('public_audio_updates', 'users.id', '=', 'public_audio_updates.user_id')->where('public_audio_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_audios.show')
                ...
                ->with('publicAudioViewsList', $publicAudioViewsList)
                ->with('publicAudioUpdatesList', $publicAudioUpdatesList)
                ->with('user', $user)
                ->with('user_id', $user_id)
                ->with('now', $now)
    ...
    public function update($id, UpdatePublicAudioRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicAudio = $this->publicAudioRepository->update($request->all(), $id);

                DB::table('public_audio_updates')->insert(['actual_name' => $newPublicAudio -> name, 'past_name' => $publicAudio -> name, 'datetime' => $now, 'public_audio_id' => $id, 'user_id' => $user_id]);
