    ...
    public function update($id, UpdateUserCollegeTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($collegeTSGaleryImages as $collegeTSGaleryImage)
                {
                    $userCollegeTSGImageInserts = DB::table('user_college_t_s_galery_images')->where('college_t_s_g_image_id', $collegeTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTSGalerie -> permissions]);
