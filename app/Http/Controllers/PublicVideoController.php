...
class PublicVideoController extends AppBaseController
{
    ...
    public function store(CreatePublicVideoRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "videos/public_videos/video_" . $publicVideo -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        {         
            ...
            $publicVideoViewsList = DB::table('users')->join('public_video_views', 'users.id', '=', 'public_video_views.user_id')->where('public_video_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicVideoUpdatesList = DB::table('users')->join('public_video_updates', 'users.id', '=', 'public_video_updates.user_id')->where('public_video_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_videos.show')
                ...
                ->with('publicVideoViewsList', $publicVideoViewsList)
                ->with('publicVideoUpdatesList', $publicVideoUpdatesList)
                ->with('user', $user)
                ->with('user_id', $user_id)
                ->with('now', $now)
    ...
    public function update($id, UpdatePublicVideoRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicVideo = $this->publicVideoRepository->update($request->all(), $id);

                DB::table('public_video_updates')->insert(['actual_name' => $newPublicVideo -> name, 'past_name' => $publicVideo -> name, 'datetime' => $now, 'public_video_id' => $id, 'user_id' => $user_id]);
