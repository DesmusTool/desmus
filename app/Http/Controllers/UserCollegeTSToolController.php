    ...
    public function update($id, UpdateUserCollegeTSToolRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($collegeTSToolFiles as $collegeTSToolFile)
                {
                    $userCollegeTSToolFileInserts = DB::table('user_college_t_s_tool_files')->where('college_t_s_t_file_id', $collegeTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTSTool -> permissions]);
