...
class PublicFileController extends AppBaseController
{
    ...
    public function store(CreatePublicFileRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "files/public_files/file_" . $publicFile -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        {       
            ...
            $publicFileViewsList = DB::table('users')->join('public_file_views', 'users.id', '=', 'public_file_views.user_id')->where('public_file_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicFileUpdatesList = DB::table('users')->join('public_file_updates', 'users.id', '=', 'public_file_updates.user_id')->where('public_file_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_files.show')
                ...
                ->with('publicFileViewsList', $publicFileViewsList)
                ->with('publicFileUpdatesList', $publicFileUpdatesList)
                ->with('user_id', $user_id)
                ->with('now', $now)
                ->with('user', $user)
    ...
    public function update($id, UpdatePublicFileRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicFile = $this->publicFileRepository->update($request->all(), $id);

                DB::table('public_file_updates')->insert(['actual_name' => $newPublicFile -> name, 'past_name' => $publicFile -> name, 'datetime' => $now, 'public_file_id' => $id, 'user_id' => $user_id]);
