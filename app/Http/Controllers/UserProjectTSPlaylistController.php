    ...
    public function update($id, UpdateUserProjectTSPlaylistRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($projectTSPlaylistAudios as $projectTSPlaylistAudio)
                {
                    $userProjectTSPlaylistAudioInserts = DB::table('user_project_t_s_p_audios')->where('p_t_s_p_a_id', $projectTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTSPlaylist -> permissions]);
