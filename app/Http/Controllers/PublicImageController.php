...
class PublicImageController extends AppBaseController
{
    ...
    public function store(CreatePublicImageRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "images/public_images/image_" . $publicImage -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        {       
            ...
            $publicImageViewsList = DB::table('users')->join('public_image_views', 'users.id', '=', 'public_image_views.user_id')->where('public_image_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicImageUpdatesList = DB::table('users')->join('public_image_updates', 'users.id', '=', 'public_image_updates.user_id')->where('public_image_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_images.show')
                ...
                ->with('publicImageViewsList', $publicImageViewsList)
                ->with('publicImageUpdatesList', $publicImageUpdatesList)
                ->with('user', $user)
                ->with('user_id', $user_id)
                ->with('now', $now)
    ...
    public function update($id, UpdatePublicImageRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicImage = $this->publicImageRepository->update($request->all(), $id);

                DB::table('public_image_updates')->insert(['actual_name' => $newPublicImage -> name, 'past_name' => $publicImage -> name, 'datetime' => $now, 'public_image_id' => $id, 'user_id' => $user_id]);
