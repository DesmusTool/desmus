    ...
    public function update($id, UpdateUserPersonalDataTSPRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($personalDataTSPlaylistAudios as $personalDataTSPlaylistAudio)
                {
                    $userPersonalDataTSPlaylistAudioInserts = DB::table('user_p_d_t_s_p_audios')->where('p_d_t_s_p_a_id', $personalDataTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userPersonalDataTSP -> permissions]);
