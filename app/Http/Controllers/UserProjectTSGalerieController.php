    ...
    public function update($id, UpdateUserProjectTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($projectTSGaleryImages as $projectTSGaleryImage)
                {
                    $userProjectTSGImageInserts = DB::table('user_project_t_s_galery_images')->where('project_t_s_g_image_id', $projectTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTSGalerie -> permissions]);
