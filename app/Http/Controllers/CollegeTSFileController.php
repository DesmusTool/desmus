...
use App\Http\Requests\CreateCollegeTSFileRequest;
use App\Http\Requests\UpdateCollegeTSFileRequest;
use App\Repositories\CollegeTSFileRepository;
...
class CollegeTSFileController extends AppBaseController
{
    private $collegeTSFileRepository;

    public function __construct(CollegeTSFileRepository $collegeTSFileRepo)
    {
        $this->collegeTSFileRepository = $collegeTSFileRepo;
    }
    ...
    public function index(Request $request)
    {
        ...
        {
            $this->collegeTSFileRepository->pushCriteria(new RequestCriteria($request));
            $collegeTSFiles = $this->collegeTSFileRepository->all();
    
            return view('college_t_s_files.index')
    ...
    public function create($id)
    {
        if(Auth::user() != null)
        {
            ...
            return view('college_t_s_files.create')
                ->with('id', $id)
    ...
    public function store(CreateCollegeTSFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            $input = $request->all();
            $collegeTSFile = $this->collegeTSFileRepository->create($input);
    
            $file = $request->file('file');
            $new_file = 'file_' . $collegeTSFile -> id . '.' . $file -> getClientOriginalExtension();
            $file->move(public_path("files/colleges/"), $new_file);
            $fileType = $file -> getClientOriginalExtension();
            $file_path = "files/colleges/file_" . $collegeTSFile -> id . '.' . $file -> getClientOriginalExtension();
            $size = filesize($file_path);
    
            DB::table('college_t_s_files')->where('id', $collegeTSFile->id)->update(['file_type' => $fileType, 'file_size' => $size]);
            DB::table('college_t_s_file_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_file_id' => $collegeTSFile -> id]);
            DB::table('recent_activities')->insert(['name' => $collegeTSFile -> name, 'status' => 'active', 'type' => 'c_t_s_f_c', 'user_id' => $user_id, 'entity_id' => $collegeTSFile -> id, 'created_at' => $now]);
    
            Flash::success('College T S File saved successfully.');
            return redirect(route('collegeTopicSections.show', [$collegeTSFile -> college_topic_section_id]));
        }
    ...
    public function show($id)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            $user_id = Auth::user()->id;
            $collegeTSFile = $this->collegeTSFileRepository->findWithoutFail($id);
            
            if(empty($collegeTSFile))
            {
                Flash::error('College T S File not found');
                return redirect(route('collegeTSFiles.index'));
            }
            ...
            {
                DB::table('college_t_s_file_views')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_file_id' => $id]);
                DB::table('college_t_s_files')->where('id', $id)->update(['views_quantity' => DB::raw('views_quantity + 1')]);
                
                $collegeTSFile = $this->collegeTSFileRepository->findWithoutFail($id);
                $collegeTopicSectionFileViews = DB::table('users')->join('college_t_s_file_views', 'users.id', '=', 'college_t_s_file_views.user_id')->where('college_t_s_file_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                $collegeTopicSectionFileUpdates = DB::table('users')->join('college_t_s_file_updates', 'users.id', '=', 'college_t_s_file_updates.user_id')->where('college_t_s_file_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                ...
                return view('college_t_s_files.show')
                    ->with('collegeTSFile', $collegeTSFile)
                    ->with('collegeTSFileViews', $collegeTopicSectionFileViews)
                    ->with('collegeTSFileUpdates', $collegeTopicSectionFileUpdates)
                    ->with('user', $user)
    ...
    public function edit($id)
    {
        if(Auth::user() != null)
        {
            ...
            foreach($userCollegeTSFiles as $userCollegeTSFile)
            {
                if($userCollegeTSFile -> user_id == $user_id && $userCollegeTSFile -> permissions == 'advanced')
                {
                    $isShared = true;
                    break;
                }
            ...
            if($user_id == $user[0] -> id || $isShared)
            {
                ...
                return view('college_t_s_files.edit')
                    ->with('collegeTSFile', $collegeTSFile)
                    ->with('id', $id)
                    ->with('userCollegeTSFilesList', $userCollegeTSFilesList)
                    ->with('collegeTSFileViewsList', $collegeTSFileViewsList)
    ...
    public function update($id, UpdateCollegeTSFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            {
                $newCollegeTSFile = $this->collegeTSFileRepository->update($request->all(), $id);
                
                DB::table('college_t_s_files')->where('id', $id)->update(['updates_quantity' => DB::raw('updates_quantity + 1')]);
                DB::table('college_t_s_file_updates')->insert(['actual_name' => $newCollegeTSFile -> name, 'past_name' => $collegeTSFile -> name, 'datetime' => $now, 'college_t_s_file_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $collegeTSFile -> name, 'status' => 'active', 'type' => 'c_t_s_f_u', 'user_id' => $user_id, 'entity_id' => $collegeTSFile -> id, 'created_at' => $now]);
            
                Flash::success('College T S File updated successfully.');
                return redirect(route('collegeTSFiles.show', [$id]));
            }
    ...
    public function destroy($id)
    {
        if(Auth::user() != null)
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                $this->collegeTSFileRepository->delete($id);
                
                DB::table('college_t_s_file_deletes')->insert(['datetime' => $now, 'user_id' => $user_id, 'college_t_s_file_id' => $collegeTSFile -> id]);
                DB::table('recent_activities')->insert(['name' => $collegeTSFile -> name, 'status' => 'active', 'type' => 'c_t_s_f_d', 'user_id' => $user_id, 'entity_id' => $collegeTSFile -> id, 'created_at' => $now]);
            
                Flash::success('College T S File deleted successfully.');
                return redirect(route('collegeTopicSections.show', [$collegeTSFile -> college_topic_section_id]));
            }
