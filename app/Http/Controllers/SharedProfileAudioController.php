...
class SharedProfileAudioController extends AppBaseController
{
    ...
    public function store(CreateSharedProfileAudioRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "audios/shared_profile_audios/audio_" . $sharedProfileAudio -> id . '.' . $file -> getClientOriginalExtension();
            $size = filesize($file_path);
    
            DB::table('shared_profile_audio')->where('id', $sharedProfileAudio->id)->update(['file_type' => $fileType, 'file_size' => $size]);
            DB::table('recent_activities')->insert(['name' => $sharedProfileAudio -> name, 'status' => 'active', 'type' => 's_p_a_c', 'user_id' => $user_id, 'entity_id' => $sharedProfileAudio -> id, 'created_at' => $now]);
    ...
    public function show($id)
    {
        ...
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $sharedProfileAudio -> user_id || $isShared)
            {
                ...
                $sharedProfileAudioViewsList = DB::table('users')->join('shared_profile_audio_views', 'users.id', '=', 'shared_profile_audio_views.user_id')->where('s_p_a_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                $sharedProfileAudioUpdatesList = DB::table('users')->join('shared_profile_audio_updates', 'users.id', '=', 'shared_profile_audio_updates.user_id')->where('s_p_a_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                ...
                return view('shared_profile_audios.show')
                    ...
                    ->with('sharedProfileAudioViewsList', $sharedProfileAudioViewsList)
                    ->with('sharedProfileAudioUpdatesList', $sharedProfileAudioUpdatesList)
                    ->with('user', $user)
                    ->with('user_id', $user_id)
                    ->with('now', $now)
    ...
    public function update($id, UpdateSharedProfileAudioRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newSharedProfileAudio = $this->sharedProfileAudioRepository->update($request->all(), $id);

                DB::table('shared_profile_audio_updates')->insert(['actual_name' => $newSharedProfileAudio -> name, 'past_name' => $sharedProfileAudio -> name, 'datetime' => $now, 's_p_a_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $sharedProfileAudio -> name, 'status' => 'active', 'type' => 's_p_a_u', 'user_id' => $user_id, 'entity_id' => $sharedProfileAudio -> id, 'created_at' => $now]);
    ...
    public function destroy($id)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                DB::table('recent_activities')->insert(['name' => $sharedProfileAudio -> name, 'status' => 'active', 'type' => 's_p_a_d', 'user_id' => $user_id, 'entity_id' => $sharedProfileAudio -> id, 'created_at' => $now]);
