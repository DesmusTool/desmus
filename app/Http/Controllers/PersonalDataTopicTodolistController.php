...
class PersonalDataTopicTodolistController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        if(Auth::user() != null && Auth::user()->email == 'josemsoberonpenaloza@gmail.com')
        {
            ...
            $personalDataTopicTodolists = DB::table('personal_datas')->join('personal_data_topics', 'personal_datas.id', '=', 'personal_data_topics.personal_data_id')->join('personal_data_topic_todolists', 'personal_data_topics.id', '=', 'personal_data_topic_todolists.personal_data_topic_id')->where('personal_datas.user_id', '=', $user_id)->where(function ($query) {$query->where('personal_data_topic_todolists.deleted_at', '=', null);})->where(function ($query) {$query->where('personal_data_topic_todolists.status', '=', 'active');})->orderBy('personal_data_topic_todolists.datetime', 'desc')->limit(50)->get();
    ...
    {
        $user_id = Auth::user()->id;
        $user = DB::table('personal_data_topics')->join('personal_datas', 'personal_data_topics.personal_data_id', '=', 'personal_datas.id')->join('users', 'personal_datas.user_id', '=', 'users.id')->where('personal_data_topics.id', $request -> personal_data_topic_id)->get();
        ...
        if(Auth::user() != null && $user[0] -> id == $user_id)
        {
            ...
            return redirect(route('personalDataTopics.show', [$personalDataTopicTodolist -> personal_data_topic_id]));
        }
    ...
    {
        $user_id = Auth::user()->id;
        $user = DB::table('personal_data_topic_todolists')->join('personal_data_topics', 'personal_data_topic_todolists.personal_data_topic_id', '=', 'personal_data_topics.id')->join('personal_datas', 'personal_data_topics.personal_data_id', '=', 'personal_datas.id')->join('users', 'personal_datas.user_id', '=', 'users.id')->where('personal_data_topic_id', $request -> p_d_t_id)->get();
    ...
    {
        $user_id = Auth::user()->id;
        $user = DB::table('personal_data_topic_todolists')->join('personal_data_topics', 'personal_data_topic_todolists.personal_data_topic_id', '=', 'personal_data_topics.id')->join('personal_datas', 'personal_data_topics.personal_data_id', '=', 'personal_datas.id')->join('users', 'personal_datas.user_id', '=', 'users.id')->where('personal_data_topic_todolists.id', $id)->get();
