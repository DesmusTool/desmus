...
class PublicAdvertisementController extends AppBaseController
{
    ...
    public function store(CreatePublicAdvertisementRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $image_path = "images/public_advertisement_images/image_" . $publicAdvertisement -> id . '.' . $image -> getClientOriginalExtension();
            ...
            $video_path = "videos/public_advertisement_videos/video_" . $publicAdvertisement -> id . '.' . $video -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        {       
            ...
            $publicAdvertisementViewsList = DB::table('users')->join('public_advertisement_views', 'users.id', '=', 'public_advertisement_views.user_id')->where('public_advertisement_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicAdvertisementUpdatesList = DB::table('users')->join('public_advertisement_updates', 'users.id', '=', 'public_advertisement_updates.user_id')->where('public_advertisement_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_advertisements.show')
                ...
                ->with('publicAdvertisementViewsList', $publicAdvertisementViewsList)
                ->with('publicAdvertisementUpdatesList', $publicAdvertisementUpdatesList)
                ->with('user', $user)
                ->with('user_id', $user_id)
                ->with('now', $now)
    ...
    public function update($id, UpdatePublicAdvertisementRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicAdvertisement = $this->publicAdvertisementRepository->update($request->all(), $id);

                DB::table('public_advertisement_updates')->insert(['actual_name' => $newPublicAdvertisement -> name, 'past_name' => $publicAdvertisement -> name, 'datetime' => $now, 'public_advertisement_id' => $id, 'user_id' => $user_id]);
