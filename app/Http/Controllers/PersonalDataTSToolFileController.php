...
use App\Http\Requests\CreatePersonalDataTSToolFileRequest;
use App\Http\Requests\UpdatePersonalDataTSToolFileRequest;
use App\Repositories\PersonalDataTSToolFileRepository;
...
class PersonalDataTSToolFileController extends AppBaseController
{
    private $personalDataTSToolFileRepository;

    public function __construct(PersonalDataTSToolFileRepository $personalDataTSToolFileRepo)
    {
        $this->personalDataTSToolFileRepository = $personalDataTSToolFileRepo;
    }
    ...
    public function index(Request $request)
    {
        ...
        {
            $this->personalDataTSToolFileRepository->pushCriteria(new RequestCriteria($request));
            $personalDataTSToolFiles = $this->personalDataTSToolFileRepository->all();
    
            return view('personal_data_t_s_tool_files.index')
    ...
    public function create($id)
    {
        if(Auth::user() != null)
        {
            ...
            return view('personal_data_t_s_tool_files.create')
                ->with('id', $id)
    ...
    public function store(CreatePersonalDataTSToolFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            $input = $request->all();
            $personalDataTSToolFile = $this->personalDataTSToolFileRepository->create($input);
            
            $file = $request->file('file');
            $new_file = 'file_' . $personalDataTSToolFile -> id . '.' . $file -> getClientOriginalExtension();
            $file->move(public_path("tools/personal_datas/"), $new_file);
            $fileType = $file -> getClientOriginalExtension();
            $file_path = "tools/personal_datas/file_" . $personalDataTSToolFile -> id . '.' . $file -> getClientOriginalExtension();
            $size = filesize($file_path);
    
            DB::table('personal_data_t_s_tool_files')->where('id', $personalDataTSToolFile->id)->update(['file_type' => $fileType, 'file_size' => $size]);
            DB::table('personal_data_t_s_tool_file_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'p_d_t_s_t_f_id' => $personalDataTSToolFile -> id]);
            DB::table('recent_activities')->insert(['name' => $personalDataTSToolFile -> name, 'status' => 'active', 'type' => 'p_d_t_s_t_f_c', 'user_id' => $user_id, 'entity_id' => $personalDataTSToolFile -> id, 'created_at' => $now]);
    
            Flash::success('PersonalData T S Tool File saved successfully.');
            return redirect(route('personalDataTSTools.show', [$personalDataTSToolFile -> personal_data_t_s_t_id]));
        }
    ...
    public function show($id)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            $user_id = Auth::user()->id;
            $personalDataTSToolFile = $this->personalDataTSToolFileRepository->findWithoutFail($id);
            
            if(empty($personalDataTSToolFile))
            {
                Flash::error('PersonalData T S Tool File not found');
                return redirect(route('personalDataTSToolFiles.index'));
            }
            ...
            {
                DB::table('personal_data_t_s_tool_file_views')->insert(['datetime' => $now, 'user_id' => $user_id, 'personal_d_t_s_t_f_id' => $id]);
                DB::table('personal_data_t_s_tool_files')->where('id', $id)->update(['views_quantity' => DB::raw('views_quantity + 1')]);
                
                $personalDataTSToolFile = $this->personalDataTSToolFileRepository->findWithoutFail($id);
                $personalDataTopicSectionToolFileViews = DB::table('users')->join('personal_data_t_s_tool_file_views', 'users.id', '=', 'personal_data_t_s_tool_file_views.user_id')->where('personal_d_t_s_t_f_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                $personalDataTopicSectionToolFileUpdates = DB::table('users')->join('personal_data_t_s_tool_file_updates', 'users.id', '=', 'personal_data_t_s_tool_file_updates.user_id')->where('p_d_t_s_t_f_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                ...
                return view('personal_data_t_s_tool_files.show')
                    ->with('personalDataTSToolFile', $personalDataTSToolFile)
                    ->with('personalDataTSTFileViews', $personalDataTopicSectionToolFileViews)
                    ->with('personalDataTSTFileUpdates', $personalDataTopicSectionToolFileUpdates)
                    ->with('user', $user)
                    ->with('now', $now)
    ...
    public function edit($id)
    {
        if(Auth::user() != null)
        {
            ...
            foreach($userPersonalDataTSToolFiles as $userPersonalDataTSToolFile)
            {
                if($userPersonalDataTSToolFile -> user_id == $user_id && $userPersonalDataTSToolFile -> permissions == 'advanced')
                {
                    $isShared = true;
                    break;
                }
            ...
            if($user_id == $user[0] -> id || $isShared)
            {
                ...
                return view('personal_data_t_s_tool_files.edit')
                    ->with('personalDataTSToolFile', $personalDataTSToolFile)
                    ->with('userPersonalDataTSToolFilesList', $userPersonalDataTSToolFilesList)
                    ->with('personalDataTSToolFileViewsList', $personalDataTSToolFileViewsList)
                    ->with('personalDataTSToolFileUpdatesList', $personalDataTSToolFileUpdatesList);
    ...
    public function update($id, UpdatePersonalDataTSToolFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            {
                $newPersonalDataTSToolFile = $this->personalDataTSToolFileRepository->update($request->all(), $id);
        
                DB::table('personal_data_t_s_tool_files')->where('id', $id)->update(['updates_quantity' => DB::raw('updates_quantity + 1')]);
                DB::table('personal_data_t_s_tool_file_updates')->insert(['actual_name' => $newPersonalDataTSToolFile -> name, 'past_name' => $personalDataTSToolFile -> name, 'datetime' => $now, 'p_d_t_s_t_f_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $personalDataTSToolFile -> name, 'status' => 'active', 'type' => 'p_d_t_s_t_f_u', 'user_id' => $user_id, 'entity_id' => $personalDataTSToolFile -> id, 'created_at' => $now]);
            
                Flash::success('PersonalData T S Tool File updated successfully.');
                return redirect(route('personalDataTSToolFiles.show', [$id]));
            }
    ...
    public function destroy($id)
    {
        if(Auth::user() != null)
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                $this->personalDataTSToolFileRepository->delete($id);
                ...
                DB::table('recent_activities')->insert(['name' => $personalDataTSToolFile -> name, 'status' => 'active', 'type' => 'p_d_t_s_t_f_d', 'user_id' => $user_id, 'entity_id' => $personalDataTSToolFile -> id, 'created_at' => $now]);
            
                Flash::success('PersonalData T S Tool File deleted successfully.');
                return redirect(route('personalDataTSTools.show', [$personalDataTSToolFile -> personal_data_t_s_t_id]));
            }
