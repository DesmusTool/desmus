    ...
    public function update($id, UpdateUserJobTSToolRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($jobTSToolFiles as $jobTSToolFile)
                {
                    $userJobTSToolFileInserts = DB::table('user_job_t_s_tool_files')->where('job_t_s_t_file_id', $jobTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userJobTSTool -> permissions]);
