    ...
    public function update($id, UpdateUserCollegeTSPlaylistRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($collegeTSPlaylistAudios as $collegeTSPlaylistAudio)
                {
                    $userCollegeTSPlaylistAudioInserts = DB::table('user_college_t_s_p_audios')->where('c_t_s_p_a_id', $collegeTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTSPlaylist -> permissions]);
