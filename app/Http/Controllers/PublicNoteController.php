...
class PublicNoteController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        {   
            ...
            $publicNoteViewsList = DB::table('users')->join('public_note_views', 'users.id', '=', 'public_note_views.user_id')->where('public_note_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
            $publicNoteUpdatesList = DB::table('users')->join('public_note_updates', 'users.id', '=', 'public_note_updates.user_id')->where('public_note_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
        }
        ...
        if(isset(Auth::user()->id))
        {
            return view('public_notes.show')
                ...
                ->with('publicNoteCommentResponses', $public_note_comment_responses)
                ->with('publicNoteCommentResponseUsers', $public_note_comment_response_users)
                ->with('publicNoteViewsList', $publicNoteViewsList)
                ->with('publicNoteUpdatesList', $publicNoteUpdatesList)
                ->with('text', $text)
                ->with('user', $user)
                ->with('user_id', $user_id)
                ->with('now', $now)
    ...
    public function update($id, UpdatePublicNoteRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newPublicNote = $this->publicNoteRepository->update($request->all(), $id);

                DB::table('public_note_updates')->insert(['actual_name' => $newPublicNote -> name, 'past_name' => $publicNote -> name, 'datetime' => $now, 'public_note_id' => $id, 'user_id' => $user_id]);
