    ...
    public function update($id, UpdateUserJobTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($jobTSGaleryImages as $jobTSGaleryImage)
                {
                    $userJobTSGImageInserts = DB::table('user_job_t_s_galery_images')->where('job_t_s_g_image_id', $jobTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userJobTSGalerie -> permissions]);
