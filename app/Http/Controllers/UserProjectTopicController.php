                                ...
                                foreach($projectTSGaleries as $projectTSGalery)
                                {
                                    ...
                                    if($sharedProjectTSGalery->isEmpty())
                                    {
                                        ...
                                        if(isset($userProjectTSGalery[0]))
                                        {
                                            ...
                                            {
                                                $sharedProjectTSGaleryImage = DB::table('user_project_t_s_galery_images')->where('project_t_s_g_image_id', '=', $projectTSGalery -> id)->where('user_id', '=', $input["user_id"])->where(function ($query) {$query->where('deleted_at', '=', null);})->get();
        ...
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($projectTopicSections as $projectTopicSection)
                {
                    ...
                    $userProjectTopicSectionInserts = DB::table('user_project_topic_sections')->where('project_topic_section_id', $projectTopicSection -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                    ...
                    if(isset($userProjectTopicSection[0]))
                    {
                        ...
                        foreach($projectTSFiles as $projectTSFile)
                        {
                            $userProjectTSFileInserts = DB::table('user_project_t_s_files')->where('project_t_s_file_id', $projectTSFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                        ...
                        foreach($projectTSNotes as $projectTSNote)
                        {
                            $userProjectTSNoteInserts = DB::table('user_project_t_s_notes')->where('project_t_s_note_id', $projectTSNote -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                        ...
                        foreach($projectTSGaleries as $projectTSGalery)
                        {
                            ...
                            $userProjectTSGaleryInserts = DB::table('user_project_t_s_galeries')->where('project_t_s_galery_id', $projectTSGalery -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                            ...
                            if(isset($userProjectTSGalery[0]))
                            {
                                ...
                                foreach($projectTSGaleryImages as $projectTSGaleryImage)
                                {
                                    $userProjectTSGImageInserts = DB::table('user_project_t_s_galery_images')->where('project_t_s_g_image_id', $projectTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                        ...
                        foreach($projectTSPlaylists as $projectTSPlaylist)
                        {
                            ...
                            $userProjectTSPlaylistInserts = DB::table('user_project_t_s_playlists')->where('p_t_s_p_id', $projectTSPlaylist -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);

                            if($userProjectTSPlaylistInserts == 0)
                            {
                                DB::table('user_project_t_s_playlists')->insert(['datetime' => $now, 'user_id' => $user_id, 'description' => $userProject -> description, 'permissions' => 'advanced', 'project_t_s_playlist_id' => $projectTSPlaylist -> id]);
                                $userProjectTSPlaylist = DB::table('user_project_t_s_playlists')->where('p_t_s_p_id', '=', $projectTSPlaylist -> id)->orderBy('datetime', 'desc')->limit(1)->get();
                                DB::table('u_p_t_s_playlist_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'u_p_t_s_p_id' => $userProjectTSPlaylist[0] -> id]);
                            }
                            ...
                            if(isset($userProjectTSPlaylist[0]))
                            {
                                ...
                                foreach($projectTSPlaylistAudios as $projectTSPlaylistAudio)
                                {
                                    $userProjectTSPlaylistAudioInserts = DB::table('user_project_t_s_p_audios')->where('p_t_s_p_a_id', $projectTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);
                        ...
                        foreach($projectTSTools as $projectTSTool)
                        {
                            ...
                            $userProjectTSToolInserts = DB::table('user_project_t_s_tools')->where('project_t_s_tool_id', $projectTSTool -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);

                            if(!$userProjectTSToolInserts)
                            {
                            ...
                            if(isset($userProjectTSTool[0]))
                            {
                                ...
                                foreach($projectTSToolFiles as $projectTSToolFile)
                                {
                                    $userProjectTSToolFileInserts = DB::table('user_project_t_s_tool_files')->where('project_t_s_t_file_id', $projectTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTopic -> permissions]);

                                    if(!$userProjectTSToolFileInserts)
                                    {
