...
use App\Http\Requests\CreateJobTSToolFileRequest;
use App\Http\Requests\UpdateJobTSToolFileRequest;
use App\Repositories\JobTSToolFileRepository;
...
class JobTSToolFileController extends AppBaseController
{
    private $jobTSToolFileRepository;

    public function __construct(JobTSToolFileRepository $jobTSToolFileRepo)
    {
        $this->jobTSToolFileRepository = $jobTSToolFileRepo;
    }
    ...
    public function index(Request $request)
    {
        ...
        {
            $this->jobTSToolFileRepository->pushCriteria(new RequestCriteria($request));
            $jobTSToolFiles = $this->jobTSToolFileRepository->all();
    
            return view('job_t_s_tool_files.index')
    ...
    public function create($id)
    {
        if(Auth::user() != null)
        {
            ...
            return view('job_t_s_tool_files.create')
                ->with('id', $id)
    ...
    public function store(CreateJobTSToolFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            $input = $request->all();
            $jobTSToolFile = $this->jobTSToolFileRepository->create($input);
            
            $file = $request->file('file');
            $new_file = 'file_' . $jobTSToolFile -> id . '.' . $file -> getClientOriginalExtension();
            $file->move(public_path("tools/jobs/"), $new_file);
            $fileType = $file -> getClientOriginalExtension();
            $file_path = "tools/jobs/file_" . $jobTSToolFile -> id . '.' . $file -> getClientOriginalExtension();
            $size = filesize($file_path);
    
            DB::table('job_t_s_tool_files')->where('id', $jobTSToolFile->id)->update(['file_type' => $fileType, 'file_size' => $size]);
            DB::table('job_t_s_tool_file_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'job_t_s_t_file_id' => $jobTSToolFile -> id]);
            DB::table('recent_activities')->insert(['name' => $jobTSToolFile -> name, 'status' => 'active', 'type' => 'j_t_s_t_f_c', 'user_id' => $user_id, 'entity_id' => $jobTSToolFile -> id, 'created_at' => $now]);
    
            Flash::success('Job T S Tool File saved successfully.');
            return redirect(route('jobTSTools.show', [$jobTSToolFile -> job_t_s_t_id]));
        }
    ...
    public function show($id)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            $user_id = Auth::user()->id;
            $jobTSToolFile = $this->jobTSToolFileRepository->findWithoutFail($id);
            
            if(empty($jobTSToolFile))
            {
                Flash::error('Job T S Tool File not found');
                return redirect(route('jobTSToolFiles.index'));
            }
            ...
            {
                DB::table('job_t_s_tool_file_views')->insert(['datetime' => $now, 'user_id' => $user_id, 'job_t_s_t_file_id' => $id]);
                DB::table('job_t_s_tool_files')->where('id', $id)->update(['views_quantity' => DB::raw('views_quantity + 1')]);
                
                $jobTSToolFile = $this->jobTSToolFileRepository->findWithoutFail($id);
                $jobTSToolFileViews = DB::table('users')->join('job_t_s_tool_file_views', 'users.id', '=', 'job_t_s_tool_file_views.user_id')->where('job_t_s_t_file_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                $jobTSToolFileUpdates = DB::table('users')->join('job_t_s_tool_file_updates', 'users.id', '=', 'job_t_s_tool_file_updates.user_id')->where('job_t_s_t_file_id', $id)->orderBy('datetime', 'desc')->limit(50)->get();
                ...
                return view('job_t_s_tool_files.show')
                    ->with('jobTSToolFile', $jobTSToolFile)
                    ->with('jobTSTFileViews', $jobTSToolFileViews)
                    ->with('jobTSTFileUpdates', $jobTSToolFileUpdates)
                    ->with('user', $user)
                    ->with('now', $now)
    ...
    public function edit($id)
    {
        if(Auth::user() != null)
        {
            ...
            foreach($userJobTSToolFiles as $userJobTSToolFile)
            {
                if($userJobTSToolFile -> user_id == $user_id && $userJobTSToolFile -> permissions == 'advanced')
                {
                    $isShared = true;
                    break;
                }
            ...
            if($user_id == $user[0] -> id || $isShared)
            {
                ...
                return view('job_t_s_tool_files.edit')
                    ->with('jobTSToolFile', $jobTSToolFile)
                    ->with('userJobTSToolFilesList', $userJobTSToolFilesList)
                    ->with('jobTSToolFileViewsList', $jobTSToolFileViewsList)
                    ->with('jobTSToolFileUpdatesList', $jobTSToolFileUpdatesList);
    ...
    public function update($id, UpdateJobTSToolFileRequest $request)
    {
        if(Auth::user() != null)
        {
            $now = Carbon::now();
            ...
            {
                $newJobTSToolFile = $this->jobTSToolFileRepository->update($request->all(), $id);
        
                DB::table('job_t_s_tool_files')->where('id', $id)->update(['updates_quantity' => DB::raw('updates_quantity + 1')]);
                DB::table('job_t_s_tool_file_updates')->insert(['actual_name' => $newJobTSToolFile -> name, 'past_name' => $jobTSToolFile -> name, 'datetime' => $now, 'job_t_s_t_file_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $jobTSToolFile -> name, 'status' => 'active', 'type' => 'j_t_s_t_f_u', 'user_id' => $user_id, 'entity_id' => $jobTSToolFile -> id, 'created_at' => $now]);
            
                Flash::success('Job T S Tool File updated successfully.');
                return redirect(route('jobTSToolFiles.show', [$id]));
            }
    ...
    public function destroy($id)
    {
        if(Auth::user() != null)
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                $this->jobTSToolFileRepository->delete($id);
                
                DB::table('job_t_s_tool_file_deletes')->insert(['datetime' => $now, 'user_id' => $user_id, 'job_t_s_t_file_id' => $jobTSToolFile -> id]);
                DB::table('recent_activities')->insert(['name' => $jobTSToolFile -> name, 'status' => 'active', 'type' => 'j_t_s_t_f_d', 'user_id' => $user_id, 'entity_id' => $jobTSToolFile -> id, 'created_at' => $now]);
            
                Flash::success('Job T S Tool File deleted successfully.');
                return redirect(route('jobTSTools.show', [$jobTSToolFile -> job_t_s_t_id]));
            }
