...
class SharedProfileVideoController extends AppBaseController
{
    ...
    public function store(CreateSharedProfileVideoRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "videos/shared_profile_videos/video_" . $sharedProfileVideo -> id . '.' . $file -> getClientOriginalExtension();
            $size = filesize($file_path);
    
            DB::table('shared_profile_video')->where('id', $sharedProfileVideo->id)->update(['file_type' => $fileType, 'file_size' => $size]);
            DB::table('recent_activities')->insert(['name' => $sharedProfileVideo -> name, 'status' => 'active', 'type' => 's_p_v_c', 'user_id' => $user_id, 'entity_id' => $sharedProfileVideo -> id, 'created_at' => $now]);
    ...
    public function show($id)
    {
        ...
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $sharedProfileVideo -> user_id || $isShared)
            {
                ...
                $sharedProfileVideoViewsList = DB::table('users')->join('shared_profile_video_views', 'users.id', '=', 'shared_profile_video_views.user_id')->where('s_p_v_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                $sharedProfileVideoUpdatesList = DB::table('users')->join('shared_profile_video_updates', 'users.id', '=', 'shared_profile_video_updates.user_id')->where('s_p_v_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                ...
                return view('shared_profile_videos.show')
                    ...
                    ->with('sharedProfileVideoViewsList', $sharedProfileVideoViewsList)
                    ->with('sharedProfileVideoUpdatesList', $sharedProfileVideoUpdatesList)
                    ->with('user', $user)
                    ->with('user_id', $user_id)
                    ->with('now', $now)
    ...
    public function update($id, UpdateSharedProfileVideoRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newSharedProfileVideo = $this->sharedProfileVideoRepository->update($request->all(), $id);

                DB::table('shared_profile_video_updates')->insert(['actual_name' => $newSharedProfileVideo -> name, 'past_name' => $sharedProfileVideo -> name, 'datetime' => $now, 's_p_v_id' => $id, 'user_id' => $user_id]);
                DB::table('recent_activities')->insert(['name' => $sharedProfileVideo -> name, 'status' => 'active', 'type' => 's_p_v_u', 'user_id' => $user_id, 'entity_id' => $sharedProfileVideo -> id, 'created_at' => $now]);
    ...
    public function destroy($id)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                ...
                DB::table('recent_activities')->insert(['name' => $sharedProfileVideo -> name, 'status' => 'active', 'type' => 's_p_v_d', 'user_id' => $user_id, 'entity_id' => $sharedProfileVideo -> id, 'created_at' => $now]);
