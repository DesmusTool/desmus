                            ...
                            if(isset($userCollegeTopicSection[0]))
                            {
                                ...
                                foreach($collegeTSPlaylists as $collegeTSPlaylist)
                                {
                                    ...
                                    if($sharedCollegeTSPlaylist->isEmpty())
                                    {
                                        ...
                                        {
                                            $sharedCollegeTSPlaylistAudio = DB::table('user_college_t_s_p_audios')->where('c_t_s_p_a_id', '=', $collegeTSPlaylist -> id)->where('user_id', '=', $input["user_id"])->where(function ($query) {$query->where('deleted_at', '=', null);})->get();
        ...
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($collegeTopicSections as $collegeTopicSection)
                {
                    ...
                    $userCollegeTopicSectionInserts = DB::table('user_college_topic_sections')->where('college_topic_section_id', $collegeTopicSection -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                    ...
                    if(isset($userCollegeTopicSection[0]))
                    {
                        ...
                        foreach($collegeTSFiles as $collegeTSFile)
                        {
                            $userCollegeTSFileInserts = DB::table('user_college_t_s_files')->where('college_t_s_file_id', $collegeTSFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                        ...
                        foreach($collegeTSNotes as $collegeTSNote)
                        {
                            $userCollegeTSNoteInserts = DB::table('user_college_t_s_notes')->where('college_t_s_note_id', $collegeTSNote -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                        ...
                        foreach($collegeTSGaleries as $collegeTSGalery)
                        {
                            ...
                            $userCollegeTSGaleryInserts = DB::table('user_college_t_s_galeries')->where('college_t_s_galery_id', $collegeTSGalery -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                            ...
                            if(isset($userCollegeTSGalery[0]))
                            {
                                ...
                                foreach($collegeTSGaleryImages as $collegeTSGaleryImage)
                                {
                                    $userCollegeTSGImageInserts = DB::table('user_college_t_s_galery_images')->where('college_t_s_g_image_id', $collegeTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                        ...
                        foreach($collegeTSPlaylists as $collegeTSPlaylist)
                        {
                            ...
                            $userCollegeTSPlaylistInserts = DB::table('user_college_t_s_playlists')->where('c_t_s_p_id', $collegeTSPlaylist -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);

                            if($userCollegeTSPlaylistInserts == 0)
                            {
                                DB::table('user_college_t_s_playlists')->insert(['datetime' => $now, 'user_id' => $user_id, 'description' => $userCollege -> description, 'permissions' => 'advanced', 'college_t_s_playlist_id' => $collegeTSPlaylist -> id]);
                                $userCollegeTSPlaylist = DB::table('user_college_t_s_playlists')->where('c_t_s_p_id', '=', $collegeTSPlaylist -> id)->orderBy('datetime', 'desc')->limit(1)->get();
                                DB::table('u_c_t_s_playlist_creates')->insert(['datetime' => $now, 'user_id' => $user_id, 'u_c_t_s_p_id' => $userCollegeTSPlaylist[0] -> id]);
                            }
                            ...
                            if(isset($userCollegeTSPlaylist[0]))
                            {
                                ...
                                foreach($collegeTSPlaylistAudios as $collegeTSPlaylistAudio)
                                {
                                    $userCollegeTSPlaylistAudioInserts = DB::table('user_college_t_s_p_audios')->where('c_t_s_p_a_id', $collegeTSPlaylistAudio -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                        ...
                        foreach($collegeTSTools as $collegeTSTool)
                        {
                            ...
                            $userCollegeTSToolInserts = DB::table('user_college_t_s_tools')->where('college_t_s_tool_id', $collegeTSTool -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
                            ...
                            if(isset($userCollegeTSTool[0]))
                            {
                                ...
                                foreach($collegeTSToolFiles as $collegeTSToolFile)
                                {
                                    $userCollegeTSToolFileInserts = DB::table('user_college_t_s_tool_files')->where('college_t_s_t_file_id', $collegeTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userCollegeTopic -> permissions]);
