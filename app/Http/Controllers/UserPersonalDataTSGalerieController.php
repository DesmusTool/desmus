    ...
    public function update($id, UpdateUserPersonalDataTSGalerieRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($personalDataTSGaleryImages as $personalDataTSGaleryImage)
                {
                    $userPersonalDataTSGImageInserts = DB::table('user_personal_data_t_s_galery_images')->where('p_d_t_s_g_i_id', $personalDataTSGaleryImage -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userPersonalDataTSGalerie -> permissions]);
