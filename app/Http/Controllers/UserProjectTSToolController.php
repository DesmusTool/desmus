    ...
    public function update($id, UpdateUserProjectTSToolRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($projectTSToolFiles as $projectTSToolFile)
                {
                    $userProjectTSToolFileInserts = DB::table('user_project_t_s_tool_files')->where('project_t_s_t_file_id', $projectTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userProjectTSTool -> permissions]);
