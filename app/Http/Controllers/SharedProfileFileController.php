...
class SharedProfileFileController extends AppBaseController
{
    ...
    public function store(CreateSharedProfileFileRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "files/shared_profile_files/file_" . $sharedProfileFile -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $sharedProfileFile -> user_id || $isShared)
            {
                ...
                $sharedProfileFileViewsList = DB::table('users')->join('shared_profile_file_views', 'users.id', '=', 'shared_profile_file_views.user_id')->where('s_p_f_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                $sharedProfileFileUpdatesList = DB::table('users')->join('shared_profile_file_updates', 'users.id', '=', 'shared_profile_file_updates.user_id')->where('s_p_f_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                ...
                return view('shared_profile_files.show')
                    ...
                    ->with('sharedProfileFileViewsList', $sharedProfileFileViewsList)
                    ->with('sharedProfileFileUpdatesList', $sharedProfileFileUpdatesList)
                    ->with('user_id', $user_id)
                    ->with('now', $now)
                    ->with('user', $user)
    ...
    public function update($id, UpdateSharedProfileFileRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newSharedProfileFile = $this->sharedProfileFileRepository->update($request->all(), $id);

                DB::table('shared_profile_file_updates')->insert(['actual_name' => $newSharedProfileFile -> name, 'past_name' => $sharedProfileFile -> name, 'datetime' => $now, 's_p_f_id' => $id, 'user_id' => $user_id]);
