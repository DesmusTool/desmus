...
class SharedProfileImageController extends AppBaseController
{
    ...
    public function store(CreateSharedProfileImageRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            $file_path = "images/shared_profile_images/image_" . $sharedProfileImage -> id . '.' . $file -> getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $sharedProfileImage -> user_id || $isShared)
            {
                ...
                $sharedProfileImageViewsList = DB::table('users')->join('shared_profile_image_views', 'users.id', '=', 'shared_profile_image_views.user_id')->where('s_p_i_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                $sharedProfileImageUpdatesList = DB::table('users')->join('shared_profile_image_updates', 'users.id', '=', 'shared_profile_image_updates.user_id')->where('s_p_i_id', $id)->orderBy('datetime', 'desc')->limit(10)->get();
                ...
                return view('shared_profile_images.show')
                    ...
                    ->with('sharedProfileImageViewsList', $sharedProfileImageViewsList)
                    ->with('sharedProfileImageUpdatesList', $sharedProfileImageUpdatesList)
                    ->with('user', $user)
                    ->with('user_id', $user_id)
                    ->with('now', $now)
    ...
    public function update($id, UpdateSharedProfileImageRequest $request)
    {
        if(isset(Auth::user()->id))
        {
            ...
            if($user_id == $user[0] -> id)
            {
                $newSharedProfileImage = $this->sharedProfileImageRepository->update($request->all(), $id);

                DB::table('shared_profile_image_updates')->insert(['actual_name' => $newSharedProfileImage -> name, 'past_name' => $sharedProfileImage -> name, 'datetime' => $now, 's_p_i_id' => $id, 'user_id' => $user_id]);
