    ...
    public function update($id, UpdateUserPersonalDataTSToolRequest $request)
    {
        if(Auth::user() != null)
        {
            ...
            if($user[0] -> user_id == $user_id)
            {
                ...
                foreach($personalDataTSToolFiles as $personalDataTSToolFile)
                {
                    $userPersonalDataTSToolFileInserts = DB::table('user_personal_data_t_s_tool_files')->where('personal_d_t_s_t_f_id', $personalDataTSToolFile -> id)->where('user_id', '=', $user_id)->update(['permissions' => $userPersonalDataTSTool -> permissions]);
